...
The Home page now scans the current user's Copilot installation folders on application startup. Tupi inventories installed marketplaces from `%USERPROFILE%\.copilot\installed-plugins` by treating each direct child directory as one installed marketplace record, persists that inventory in SQLite, and marks a marketplace as trusted only when its directory name matches a catalog-listed trusted marketplace. Direct child plugin folders are listed under each marketplace; skill folders under each plugin's `skills/` directory are listed separately when they contain a `SKILL.md` file. Agent files directly inside each plugin's `agents/` directory are listed under the owning plugin when they end in `*.agent.md`, with their filename-derived name, file location, and optional frontmatter `description`. Tupi also scans `%USERPROFILE%\.copilot\agents`, stores each discovered `*.agent.md` file with its filename-derived agent name, file location, optional frontmatter `description`, and a trust flag, and marks a global agent as trusted only when its file bytes exactly match a catalog-listed trusted agent file already present in Tupi's trusted marketplace cache.
